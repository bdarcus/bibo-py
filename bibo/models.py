...
class List(rdfSubject):
    rdf_type = RS['List']
    contains = rdfMultiple(RS.contains, range_type=BIBO.Document or RS.Section)
...
class Section(rdfSubject):
    rdf_type = RS['Section']
    contains = rdfMultiple(RS.contains, range_type=BIBO.Document)
    created = rdfSingle(RS.created)
...
class Item(rdfSubject):
    rdf_type = RS['Item']
    resource = rdfSingle(RS.resource, range_type=BIBO.Document)
    next = rdfSingle(RS.next)
    previous = rdfSingle(RS.previous)
